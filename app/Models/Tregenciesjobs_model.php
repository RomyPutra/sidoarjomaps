...
class Tregenciesjobs_model extends Model
{
    protected $table = 'regenciesjobs';
    ...
    {
        $scol = array('regenciesjobs.id','regenciesjobs.regency_id','regencies.name','regenciesjobs.thndata','regenciesjobs.pekerjaan','regenciesjobs.jumlah');
        $this->dt->select($scol);
        $this->dt->join('regencies', 'regencies.id = regenciesjobs.regency_id', 'left');;
        $this->dt->where('regenciesjobs.id', $id);
        $this->dt->orderBy('regencies.name', 'ASC');
        $this->dt->orderBy('regenciesjobs.pekerjaan', 'ASC');
        $query = $this->dt->get();
    ...
    {
        $scol = array('regenciesjobs.id','regenciesjobs.regency_id','regencies.name','regenciesjobs.thndata','regenciesjobs.pekerjaan','regenciesjobs.jumlah');
        $tbl_storage = $this->db->table($this->table)->select($scol)->join('regencies', 'regencies.id = regenciesjobs.regency_id', 'left');;
