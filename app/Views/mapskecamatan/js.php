<script type="text/javascript">
  $(document).ready(function() {
    $('#kecamatan').on('change',function(){
      let idkec = $(this).val();
      $.ajax({
        url: "<?php echo base_url('mapskecamatan/') ?>"+$(this).val(),
        type: "POST",
        "data": {
        ...
        success: function (response) {
          ...
          let frontData = 'Kecamatan '+data.kecamatan+' memiliki luas wilayah '+data.luaswil+' dengan batas utara '+data.btsutara+', batas timur '+data.btstimur+', batas selatan '+data.btsselatan+' dan batas barat '+data.btsbarat+' Per tahun '+data.thndata+' dengan total jumlah penduduk yaitu '+data.totala+' yang terdiri dari '+data.totall+' jiwa laki-laki dan '+data.totalp+' jiwa perempuan. Saat ini di kecamatan '+data.kecamatan+' terdapat';
          let endData = '';
          let incount = 0;
          ...
          if (storedMapState) {
            ...
            center = idkec == 0 ? center : { lat: parseFloat(data.deslat), lng: parseFloat(data.deslng) };
            zoom = idkec == 0 ? parsedState.zoom : 15;
          }
