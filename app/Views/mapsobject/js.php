<script type="text/javascript">
  $(document).ready(function() {
    $('#btn').on('click', function() {
      let urls = isNullOrEmpty($('#obyek').val()) ? "<?php echo base_url('mapsobject/-') ?>" : "<?php echo base_url('mapsobject/') ?>"+$('#obyek').val();
      $.ajax({
        url: urls,
        type: "POST",
        "data": {
        ...
        success: function (response) {
          ...
          if (storedMapState) {
            ...
            center = data.data.length < 1 ? new google.maps.LatLng(parsedState.lat, parsedState.lng) : (data.data.length > 1 ? center : { lat: parseFloat(data.data[0].lat), lng: parseFloat(data.data[0].lng) });
            zoom = data.data.length < 1 ? parsedState.zoom : (data.data.length > 1 ? zoom : 15);
          }
  ...
  function initMap() {
    ...
    fetch('<?php echo base_url('ajaxmapsobj') ?>', {
      method: 'POST',
      headers: {
