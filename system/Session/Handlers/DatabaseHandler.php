...
class DatabaseHandler extends BaseHandler
{
    ...
    public function __construct(AppConfig $config, string $ipAddress)
    {
        ...
        if ($session instanceof SessionConfig) {
            ...
            $this->expiration = $config->sessionExpiration;
        }
    ...
    public function write($id, $data): bool
    {
        ...
        // log_message('info', 'DatabaseHandler.write.data: '.print_r($data, TRUE));

        if ($this->sessionID !== $id) {
            $this->rowExists = false;
            $this->sessionID = $id;
        }
        $releaseSession = $this->realeaseExpiredSession();
        // log_message('info', 'DatabaseHandler.write.releaseSession: '.print_r($releaseSession, TRUE));
        ...
        if ($this->rowExists === false) {
            ...
            $this->db->table($this->table)->where('id', $this->idPrefix . $id)->update($updateData);
        }
    ...
    {
        log_message('info', 'DatabaseHandler.destroy: '.print_r($id, TRUE));
        if ($this->lock) {
            $builder = $this->db->table($this->table)->where('id', $this->idPrefix . $id);
    ...
    protected function realeaseExpiredSession(): bool
    {
        $separator = ' ';
        $interval  = implode($separator, ['', "{$this->expiration} second", '']);
        $builder = $this->db->table($this->table)->where('timestamp <', "now() - INTERVAL {$interval}", false);
        $data = [
            'status' => 0
        ]; 
        $result = $builder->update($data);
        log_message('info', 'DatabaseHandler.realeaseExpiredSession: '.print_r($result, TRUE));
        if (!$result) {
            return $this->fail();
        }
        return true;
